/**
 * Timer de Billard Pro v4
 * Application de chronométrage pour billard International 8-ball
 */
...
class BilliardTimer {
    constructor() {
        ...
        // Gestion du touch pour le panneau
        this.panelTouch = { 
            startX: 0, 
            startY: 0, 
            moveX: 0, 
    ...
    initEventListeners() {
        ...
        this.domElements.btnExtension.addEventListener('click', () => { 
            this.playSound('click'); 
            this.useExtension(); 
        });
        this.domElements.btnNewGame.addEventListener('click', () => { 
            this.playSound('click'); 
            this.newGame(); 
        });
        ...
        this.domElements.menuArbitre.addEventListener('click', () => { 
            this.playSound('click'); 
        ...
        // Panneau d'aide
        this.domElements.btnShowHelp.addEventListener('click', () => { 
            this.playSound('click'); 
        ...
        // Panneau des règles
        if (this.domElements.btnShowRules) {
            this.domElements.btnShowRules.addEventListener('click', () => { 
                this.playSound('click'); 
                this.toggleRules(true); 
            });
            this.domElements.overlayRules.addEventListener('click', () => this.toggleRules(false));
        ...
        window.addEventListener('load', () => {
            const loader = this.domElements.loader;
            if (loader) {
                setTimeout(() => {
    ...
    async initializeAudio() {
        ...
        try {
            ...
            // Synthétiseur pour l'alerte
            this.audio.bellSynth = new Tone.MetalSynth({ 
                frequency: 300, 
                harmonicity: 5.1, 
                modulationIndex: 16, 
                envelope: { attack: 0.001, decay: 1.4, release: 0.2 }, 
                volume: -10 
            }).connect(this.audio.volumeNode);
            
            // Gong pour la fin du temps
            this.audio.gong = new Tone.FMSynth({ 
                harmonicity: 3.1, 
                modulationIndex: 20, 
            ...
            // Son de clic
            this.audio.clickSynth = new Tone.MembraneSynth({ 
                pitchDecay: 0.01, 
                octaves: 1, 
            ...
            // Compte à rebours
            this.audio.countdownSynth = new Tone.Synth({ 
    ...
    applyConfigToUI() {
        ...
        if (this.domElements.autoStartOnPlayerChange) {
            this.domElements.autoStartOnPlayerChange.value = String(this.config.autoStartOnPlayerChange);
        }
    ...
    handleConfigChange() {
        ...
        if (this.domElements.autoStartOnPlayerChange) {
            this.config.autoStartOnPlayerChange = this.domElements.autoStartOnPlayerChange.value === 'true';
        }
    ...
    selectPlayer(playerNum) {
        ...
        if (this.config.autoStartOnPlayerChange) {
            setTimeout(() => this.start(), 50);
        }
    }
    ...
    useExtension() {
        if (!this.state.isRunning || 
            this.state.isExtensionUsedForShot || 
            this.state.extensionsUsedInGame[this.state.currentPlayer]) {
            return;
        }
        ...
        // Réinitialiser les alertes si nécessaire
    ...
    async handleTimerScreenClick(e) {
        ...
        if (now - this.state.lastClickTime < 300) {
            ...
            // Simple clic avec délai pour détecter le double-clic
    ...
    // ========================================
    // SECTION: Mise à jour de l'Interface
    // ========================================
    ...
    updateDisplay() {
        ...
        // Affichage avec millisecondes sous 10 secondes
        ...
        // Mise à jour des classes CSS pour les alertes visuelles
    ...
    playSound(type) {
        ...
        if ((isAlertType && !this.config.sonAlertes) || (isClickType && !this.config.sonClics)) {
            return;
        }
        
        const now = this.audio.context.currentTime;
        
        if (this.audio.volumeNode) {
            this.audio.volumeNode.volume.value = this.config.volume;
        }
        
        switch(type) {
            case 'warning':
                this.audio.bellSynth?.triggerAttack(now);
                break;
            case 'countdown_tick':
                this.audio.countdownSynth?.triggerAttackRelease('G5', '16n', now);
                break;
            case 'gong':
                this.audio.gong?.triggerAttackRelease('A3', '0.5s', now);
                break;
            case 'click':
                this.audio.clickSynth?.triggerAttackRelease("C4", "32n", now, 0.5);
                break;
        }
    }
    ...
    // ========================================
    // SECTION: Gestion Tactile du Panneau
    // ========================================
    ...
    handlePanelTouchEnd(e) {
        ...
        // Fermer le panneau si le swipe est suffisant
